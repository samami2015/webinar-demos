"""
## Dynamically generated DAG for ingestion of customer_feedback
...
@dag(
    ...
    schedule="@daily",
    catchup=False,
    tags=["customer_feedback", "ETL", "HappyWoofs"],
    default_args={
        "owner": "Peanut",
        "retries": 3,
        "retry_delay": duration(seconds=5),
    },
    description=f"Ingest data from customer_feedback",
    doc_md=__doc__,
)
def ingest_dag():
    @task.sensor(
        task_id=f"wait_for_new_files_customer_feedback",
    ...
    @task(task_id=f"extract_customer_feedback")
    def extract(
    ...
    @task(task_id=f"verify_checksum_customer_feedback")
    def check_checksum(
    ...
    @task(task_id=f"load_customer_feedback")
    def load(
        source_file: ObjectStoragePath,
        ...
        load_file_loc = ObjectStoragePath(
            f"{base_path_load}customer_feedback/{file_name}",
    ...
    update_dataset_obj = EmptyOperator(
        task_id=f"update_dataset_customer_feedback",
        outlets=[Dataset("s3://sen-wang-bucket/load/customer_feedback")]
    )
